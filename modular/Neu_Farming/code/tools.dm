...
/obj/item/rogueweapon/thresher/afterattack(obj/target, mob/user, proximity)
	if(user.used_intent.type == /datum/intent/flailthresh)
		if(isturf(target.loc))
			...
			var/found = FALSE
			for(var/obj/item/natural/neu_chaff/C in T)
				found = TRUE
				C.thresh()
			if(found)
...
/obj/item/rogueweapon/hoe/getonmobprop(tag)
	...
	if(tag)
		switch(tag)
			if("gen")
				return list("shrink" = 0.7,"sx" = -12,"sy" = 1,"nx" = 9,"ny" = 1,"wx" = -5,"wy" = 1,"ex" = -2,"ey" = 1,"northabove" = 0,"southabove" = 1,"eastabove" = 1,"westabove" = 0,"nturn" = -38,"sturn" = 37,"wturn" = 32,"eturn" = -32,"nflip" = 0,"sflip" = 8,"wflip" = 8,"eflip" = 0)
			if("wielded")
