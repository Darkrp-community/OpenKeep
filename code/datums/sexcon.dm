...
/datum/sex_controller/male/cum(source)
	...
	switch(source)
		...
		if("fuckmouth")
			...
			owner.visible_message("<span class='notice'>[owner] feeds [fucking]!</span>")
			add_cum_floor(get_turf(fucking))
			for(var/datum/disease/A in owner.diseases)
				if(istype(A,/datum/disease/sexdisease))
					fucking.ForceContractDisease(new /datum/disease/sexdisease)
			for(var/datum/disease/A in fucking.diseases)
				if(istype(A,/datum/disease/sexdisease))
					owner.ForceContractDisease(new /datum/disease/sexdisease)
