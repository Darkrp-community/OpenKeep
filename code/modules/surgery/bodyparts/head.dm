...
/obj/item/bodypart/head/drop_limb()
...
/obj/item/bodypart/head/proc/knock_out_teeth(throw_dir, num=32) //Won't support knocking teeth out of a dismembered head or anything like that yet.
	...
	if(teeth_list && teeth_list.len) //We still have teeth
		...
		for(var/curr = 1 to stacks) //Random amount of teeth stacks
			...
			teeth.use(drop)
			T.add_mob_blood(owner)
			playsound(owner, "fracturedry", 75, 0)
...
/obj/item/stack/teeth
	...
	max_amount = 32
	gender = PLURAL
	desc = "Either the dungeoneer is getting creative or there was a fight around here."
...
/obj/item/specialtooth // special teeth, basically teeth that can be sold and shouldnt be stacked as selling stacked items is fucked with the NAVIGATOR
	name = "golden tooth"
	desc = "Nobles delight."
	icon = 'icons/obj/surgery.dmi'

/obj/item/specialtooth/sharp
	name = "fang"
	desc = ""
	icon_state = "sharpie"

/obj/item/specialtooth/sharp/Initialize()
	. = ..()
	sellprice = rand(13,26)

/obj/item/specialtooth/Initialize()
	. = ..()
	icon_state = "gold_teeth[rand(1,2)]"
	sellprice = rand(23,46)
...
/obj/item/stack/teeth/update_icon()
	...
	for(var/i = 1 to src.amount)
		var/image/I = image('icons/obj/surgery.dmi',"tooth[rand(1,4)]")
