...
/obj/structure/roguemachine/steward/Topic(href, href_list)
	...
	if(href_list["import"])
		...
		scom_announce("Blackwine imports [D.name] for [amt] mammon.", )
		D.raise_demand()
	...
	if(href_list["export"])
		...
		scom_announce("Blackwine exports [D.name] for [amt] mammon.")
		D.lower_demand()
	...
	if(href_list["payroll"])
		var/list/L = list(GLOB.noble_positions) + list(GLOB.garrison_positions) + list(GLOB.church_positions) + list(GLOB.towner_positions) + list(GLOB.peasant_positions) + list(GLOB.apprentices_positions)
