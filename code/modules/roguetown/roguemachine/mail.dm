...
/obj/structure/roguemachine/mail/attackby(obj/item/P, mob/user, params)
	if(istype(P, /obj/item/paper/confession))
		if((user.mind.assigned_role == "Templar") || (user.mind.assigned_role == "Witch Hunter"))
			...
			if(C.signed)
				...
				qdel(C)
				visible_message("<span class='warning'>[user] sends something.</span>")
				send_ooc_note("Confessions: [GLOB.confessors.len]/5", job = list("templar", "puritan", "priest"))
