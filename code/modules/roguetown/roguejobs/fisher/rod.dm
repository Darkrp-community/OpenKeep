...
/obj/item/fishingrod/attackby(obj/item/I, mob/user, params)
	...
	if(istype(I, /obj/item/fishing/bait) || istype(I, /obj/item/natural/worms) || istype(I, /obj/item/natural/bundle/worms) || I.isbait)
...
/obj/item/fishingrod/afterattack(target, mob/living/user, proximity, )
	...
	for(var/obj/item/fishing/A in attacheditems)
		...
		hookwindow += A.hookmod - round(baited.baitpenalty/5)
	...
	fishpicker = pickweightmerge(fishpicker, baited.fishloot)
	if(baited.sizemod)
		sizepicker = pickweightmerge(sizepicker, baited.sizemod)
	if(baited.raritymod)
		raritypicker = pickweightmerge(raritypicker, baited.raritymod)
		...
		switch(currentstate)
			...
			if("hooked")
				...
				if(targetdif >= 90 || targetdif <= -90)
					linehealth--
					if(targetdif >= 90 && lasttarget < 90)
						to_chat(fisher, "<span class='warning'>The line's going slack!</span>")
					else if(targetdif <= -90 && lasttarget > -90)
						to_chat(fisher, "<span class='warning'>The line's too tight!</span>")
				...
				if(!specialsize)
					switch(fishsize)
						if("tiny")
							...
							caughtfish.isbait = TRUE
							caughtfish.baitpenalty = 0
						if("small")
