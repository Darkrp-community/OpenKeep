...
/obj/item/flashlight/flare/torch/lantern/bronze/afterattack(atom/movable/A, mob/user, proximity)
    . = ..()
    if(!proximity)
        return
    if(on && (prob(50) || (user.used_intent.type == /datum/intent/use)))
...
/obj/item/flashlight/flare/torch/lantern/bronze/process()
	open_flame(heat)
	fuel = max(fuel - 1, 0)
	if(!fuel || !on)
		turn_off()
...
/obj/item/flashlight/flare/torch/lantern/bronze/extinguish()
	return

/obj/item/flashlight/flare/torch/lantern/bronze/getonmobprop(tag)
	. = ..()
	if(tag)
		switch(tag)
			if("gen")
